...
Use this when you want the **Analytics → Overview** tab to render real-looking
charts for a demo account without running the full simulation pipeline.

What it does
------------
* Resolves a student by email in Supabase (`public.users`).
* Pulls every course they're enrolled in via `public.course_enrollments`,
  along with each course's date window (`course_start_date`,
  `course_end_date`).
* For every (course, week) pair inside that window (intersected with the
  requested ``--weeks`` look-back), generates a deterministic feature
  snapshot — logins, active minutes, materials viewed, submissions,
  attendance, average score — and upserts it into
  ``learnez_ai.student_weekly_features``.
* Honors the same unique index ``(user_id, course_id, week_start)`` used by
  the production aggregator, so re-running is idempotent.

What it does NOT do
-------------------
* It does not write to ``elearning_raw.activity_events`` etc. The weekly
  collection is the only thing the model and analytics pages read; raw
  events would be needed only if you re-run the aggregator.
* It does not seed Supabase ``assignment_submissions``. The grade-distribution
  chart populates from real submissions; if you want fake grades, take an
  assignment as the student through the LMS.

Usage
-----

    cd BE
    python -m ml.data.seed_demo_student --email student1@email.com --weeks 16

    # Different persona for variety:
    python -m ml.data.seed_demo_student --email student1@email.com --persona at_risk
    python -m ml.data.seed_demo_student --email student1@email.com --persona steady --weeks 24

Personas
--------

* ``steady``   (default): healthy attendance, on-time submissions, ~70-85 grades.
* ``thriving`` : high engagement, near-perfect attendance, 80-95 grades.
* ``at_risk``  : declining engagement, missed sessions, mid-50 grades.

Re-runs are safe — existing snapshots for the same (user, course, week) are
overwritten so you can flip personas freely.
"""
...
def _week_floor_utc(d: datetime | date) -> datetime:
    """Snap any date/datetime to the Monday-00:00:00-UTC of its week."""
...
def _resolve_student(sb, email: str) -> dict[str, Any]:
    ...
    if role_id != 3:
        # role 3 == Student per ROLE_MAP; warn but allow (admins sometimes
        # promote dev accounts). We still seed.
...
def _course_window_weeks(
    ...
    *,
    ...
    """Return the list of week_start datetimes inside the course window.

    If the course has no explicit start/end, fall back to the requested
    look-back window (``fallback_start`` .. ``fallback_end``).
    """
...
def _generate_features(
    *,
    ...
    ``week_index`` / ``total_weeks`` lets us nudge the at-risk persona to
    decline over time while keeping the steady/thriving personas roughly
    flat — this mirrors what the dashboard "trend" indicators look for.
    """
    progress = week_index / max(total_weeks - 1, 1)

    # At-risk students drift downward as the term progresses; others stay
    # roughly steady with mild noise.
    drift = 1.0
    ...
    def _scaled_int(rng_: random.Random, lo: int, hi: int) -> int:
        v = rng_.randint(lo, hi)
        return max(0, int(round(v * drift)))

    def _scaled_float(rng_: random.Random, lo: float, hi: float) -> float:
        v = rng_.uniform(lo, hi)
        return max(0.0, v * drift)
    ...
    material_open_time_sec = round(
        _scaled_float(rng, *persona["material_open_sec_range"]), 2
    )
    ...
    avg_score = max(0.0, min(100.0, centre + rng.gauss(0, persona["score_jitter"]) * (1.5 - drift)))
    ...
    return {
        ...
        "avg_score_30d": round(float(avg_score), 4),
...
async def seed_student_features(
    *,
    ...
    """Reusable entrypoint used by the cohort orchestrator.

    Returns the number of weekly snapshots upserted into
    ``learnez_ai.student_weekly_features``.

    ``courses`` is the list of Supabase course rows the student is enrolled in
    (callers can resolve them via :func:`_resolve_courses`); we accept them
    pre-resolved so the orchestrator can do a single batch lookup for the
    whole cohort instead of N + 1 round-trips.
    """
    ...
    end = _week_floor_utc(end) + timedelta(days=7)  # include the current week
    ...
    for course in courses:
        ...
        if ignore_course_window:
            # Stretch the seed across the requested look-back regardless of
            # course start/end dates — handy when the course only officially
            # started a few days ago but you want a longer demo timeline.
        ...
        for idx, week_start in enumerate(week_dates):
            ...
            grand_total += 1
        if log_prefix:
            print(f"{log_prefix}[ok] course {course_id}: upserted {len(week_dates)} weekly snapshot(s)")

    return grand_total


async def _seed(args: argparse.Namespace) -> int:
    """Single-student CLI entry point. Thin wrapper over :func:`seed_student_features`."""
    ...
    print(
        "Tip: refresh the Analytics → Overview tab. The engagement chart and "
        "stat cards now read these rows; the grade distribution still depends "
        "on real graded assignment_submissions in Supabase."
    )
    return 0


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Seed demo analytics for one student.")
    p.add_argument("--email", required=True, help="Student email registered in public.users.")
    p.add_argument(
        "--weeks",
        type=int,
        default=16,
        help="Look-back window in weeks (default 16). Per-course start/end dates are intersected with this.",
    )
    p.add_argument(
        "--persona",
        choices=sorted(PERSONAS.keys()),
        default="steady",
        help="Behavioral profile to seed (default: steady).",
    )
    p.add_argument(
        "--ignore-course-window",
        action="store_true",
        help=(
            "Seed every week in the look-back window even if it predates the "
            "course's start date. Useful when the course just started but you "
            "want a longer demo timeline."
        ),
    )
    return p.parse_args()
