"""Seed persona-aware ``assignment_submissions`` (and per-question answers)
for one or more students, so the grade-distribution chart and the lecturer's
grading queue have realistic, finalised data.

Idempotency
-----------
Each ``(student_id, assignment_id)`` pair gets at most one submission. Re-runs
are safe: if a submission already exists for the pair we skip it (we never
mutate or delete graded work).

Coverage
--------
We only seed submissions for assignments whose ``due_date`` is in the past.
Future assignments are intentionally left ungraded so the lecturer's "to
grade" queue still has work to demo.

What we write
-------------
* ``assignment_submissions`` — ``status='submitted'``, ``is_corrected=true``,
  ``final_score``, ``submitted_at``, ``is_late``, ``elapsed_time``.
* ``assignment_submission_answers`` — one row per question. For MCQ we set
  ``is_correct`` + ``earned_score``; for essay/manual we set
  ``earned_score`` only.
"""
...
#
# - ``mcq_correct_prob``    : probability that any given MCQ question is correct.
# - ``score_pct_range``     : final score as a fraction of total_score for non-MCQ submissions
#                              (essay/manual). Centre + jitter is sampled inside this range.
# - ``late_prob``           : probability that the submission is late. Late submissions slot
#                              into a small post-due window.
# - ``elapsed_pct_range``   : if ``duration_enabled`` is on, fraction of duration the student
#                              actually spent (0.x .. 1.0, may exceed 1.0 for late thinkers).
# - ``early_minutes_range`` : how many minutes before the deadline the on-time submission
#                              clocks in.
GRADING_PROFILE: dict[str, dict[str, Any]] = {
    ...
    "at_risk": {
        ...
        "early_minutes_range": (-180, 30),  # often submits at the wire or after
    },
}


def _resolve_assignments_for_student(
    sb,
    *,
    course_ids: list[int],
) -> list[dict[str, Any]]:
    ...
    module_to_course = {int(m["id"]): int(m["course_id"]) for m in module_rows if m.get("id") is not None}
...
def _grade_mcq_question(question: dict[str, Any], rng) -> tuple[bool, str]:
    """Decide whether the seeded answer is correct and emit ``answer_content`` JSON."""
...
def seed_submissions_for_student(
    ...
    *,
    ...
    Returns the number of new submission rows inserted.
    """
    ...
    past_assignments = [
        a for a in assignments if (_parse_iso(a.get("due_date")) or now + timedelta(days=365)) <= now
    ]
    ...
    for assignment in past_assignments:
        ...
        if is_late:
            ...
            submitted_at = due_date - timedelta(minutes=rng.randint(int(early_lo), int(max(early_lo, early_hi))))
            if submitted_at > now:
                submitted_at = now - timedelta(minutes=rng.randint(0, 60))
            is_late = submitted_at > due_date
        ...
        if questions:
            ...
            for q in questions:
                ...
                if qtype == "mcq":
                    ...
                    answer_payloads.append(
                        {
                            "submission_id": None,  # filled in after submission insert
                            "question_id": qid,
                            "answer_content": answer_content,
                            "is_correct": bool(is_correct),
                            "earned_score": round(earned, 4),
                            "ai_feedback": None,
                        }
                    )
                else:
            ...
            final_score = round(min(earned_total, total_score), 4) if total_score else round(earned_total, 4)
        else:
            # Manual assignment (no questions). Score directly from persona band.
            pct = rng.uniform(*profile["score_pct_range"])
            final_score = round(total_score * pct, 4) if total_score else 0.0
